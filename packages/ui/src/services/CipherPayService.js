...
} from "./accountOverviewService";
import {
  encryptForRecipient,
  encryptForSender,
  getLocalEncPublicKeyB64,
} from "../lib/e2ee";
...
class CipherPayService {
  constructor() {
    ...
    this.config = {
      ...
      programId:
        ...
        "AWVNBHaF1upXopq9dQpRpY54c9113bBskqiv16MUTDDd", // Solana program ID
    ...
    try {
      ...
      if (!sdkInitialized || !sdkClass) {
        ...
        if (
          ...
          typeof window.CipherPaySDK === "function" &&
          window.CipherPaySDK.prototype &&
          window.CipherPaySDK.prototype.constructor === window.CipherPaySDK
        ) {
          sdkClass = window.CipherPaySDK;
        } else if (
          typeof window !== "undefined" &&
          typeof window.CipherPaySDK !== "undefined"
        ) {
          throw new Error(
            "CipherPaySDK found in global scope but is not a constructor class. The SDK appears to export utility functions only.",
          );
        } else {
          throw new Error(
            "CipherPay SDK not available in global scope. Ensure the SDK bundle is loaded via script tag in index.html",
          );
        }
      }
      ...
      const circuitConfig = {
        transfer: {
          wasmUrl:
            import.meta.env.VITE_TRANSFER_WASM_URL || "/circuits/transfer.wasm",
          zkeyUrl:
            import.meta.env.VITE_TRANSFER_ZKEY_URL || "/circuits/transfer.zkey",
          verificationKeyUrl:
            import.meta.env.VITE_TRANSFER_VKEY_URL ||
            "/circuits/verifier-transfer.json",
        },
        merkle: {
          wasmUrl:
            import.meta.env.VITE_MERKLE_WASM_URL || "/circuits/merkle.wasm",
          zkeyUrl:
            import.meta.env.VITE_MERKLE_ZKEY_URL || "/circuits/merkle.zkey",
          verificationKeyUrl:
            import.meta.env.VITE_MERKLE_VKEY_URL ||
            "/circuits/verifier-merkle.json",
        },
        withdraw: {
      ...
      console.log(
        "Creating SDK instance with config:",
      ...
      console.log(
        "SDK initialized. Call startEventMonitoring(recipientKey) to monitor on-chain events.",
      );
  ...
  async connectWallet(walletAddress = null) {
    ...
    try {
      ...
      if (walletAddress) {
        ...
        console.log(
          "[CipherPayService] connectWallet: Stored wallet address:",
          walletAddress,
        );
      ...
      if (this.sdk?.walletProvider?.getPublicAddress) {
        ...
        if (address) {
          ...
          console.log(
            "[CipherPayService] connectWallet: Got address from SDK:",
            address,
          );
          return address;
        }
      ...
      console.warn(
        "[CipherPayService] connectWallet: No wallet address provided and SDK has none",
      );
      return null;
    } catch (error) {
  ...
  getPublicAddress() {
    try {
      ...
      if (this.walletAddress) {
        console.log(
          "[CipherPayService] getPublicAddress: Using stored address:",
          this.walletAddress,
        );
  ...
  async getSpendableNotes() {
    try {
      ...
      const overview = await this.getAccountOverviewFromBackend({
        checkOnChain: false,
      });
      ...
      return spendable.map((n) => ({
        amount:
          typeof n.amount === "string" && n.amount.startsWith("0x")
            ? BigInt(n.amount)
            : BigInt(n.amount),
        tokenId:
          typeof n.note.tokenId === "string" && n.note.tokenId.startsWith("0x")
            ? BigInt(n.note.tokenId)
            : BigInt(n.note.tokenId),
        ownerCipherPayPubKey:
          typeof n.note.ownerCipherPayPubKey === "string" &&
          n.note.ownerCipherPayPubKey.startsWith("0x")
            ? BigInt(n.note.ownerCipherPayPubKey)
            : BigInt(n.note.ownerCipherPayPubKey),
        randomness: {
          r:
            typeof n.note.randomness.r === "string" &&
            n.note.randomness.r.startsWith("0x")
              ? BigInt(n.note.randomness.r)
              : BigInt(n.note.randomness.r),
          s: n.note.randomness.s
            ? typeof n.note.randomness.s === "string" &&
              n.note.randomness.s.startsWith("0x")
      ...
      console.error(
        "[CipherPayService] Failed to get spendable notes from backend:",
        error,
      );
      return [];
    }
  ...
  async getUserAta() {
    ...
    try {
      ...
      console.error(
        "[CipherPayService] Failed to get user ATA from DB:",
        error,
      );
      return {
        wsolAta: null,
  ...
  async createTransaction(recipientPublicKey, amount, inputNote = null) {
    ...
    try {
      ...
      if (amount < MIN_TRANSFER_AMOUNT) {
        throw new Error(
          `Minimum transfer amount is 0.001 SOL. Requested: ${(Number(amount) / 1e9).toFixed(9)} SOL`,
        );
      }
      ...
      if (!window.CipherPaySDK?.transfer) {
        throw new Error(
          "SDK transfer function not available. Ensure the SDK bundle is loaded.",
        );
      }
      ...
      const senderOwnerCipherPayPubKey = await poseidonHash([
        ownerWalletPubKey,
        ownerWalletPrivKey,
      ]);
      const senderOwnerKeyHex =
        "0x" +
        senderOwnerCipherPayPubKey.toString(16).padStart(64, "0").toLowerCase();
      ...
      if (typeof recipientPublicKey === "string") {
        ...
        normalizedRecipientKey =
          "0x" +
          recipientPublicKey.toString(16).padStart(64, "0").toLowerCase();
      }
      ...
      const validateOwnerCipherPayPubKey = (key) => {
        if (!key || typeof key !== "string") {
          return {
            valid: false,
            error: "owner_cipherpay_pub_key must be a string",
          };
        }
        ...
        if (!key.startsWith("0x")) {
          return {
            valid: false,
            error: "owner_cipherpay_pub_key must start with 0x",
          };
        }
        ...
        if (key.length !== 66) {
          return {
            valid: false,
            error: `owner_cipherpay_pub_key must be 66 characters (got ${key.length})`,
          };
        }
        ...
        if (!/^[0-9a-fA-F]+$/.test(hexPart)) {
          return {
            valid: false,
            error: "owner_cipherpay_pub_key contains invalid hex characters",
          };
        }
        ...
        try {
          ...
          if (keyBigInt === 0n) {
            return {
              valid: false,
              error: "owner_cipherpay_pub_key cannot be zero",
            };
          }
        } catch (e) {
          return {
            valid: false,
            error: `owner_cipherpay_pub_key is not a valid BigInt: ${e.message}`,
          };
        }
      ...
      const validateNoteEncPubKey = (key) => {
        ...
        if (key.length < 44 || key.length > 48) {
          return {
            valid: false,
            error: `note_enc_pub_key must be 44-48 characters (got ${key.length})`,
          };
        }
        ...
        try {
          ...
          if (bytes.length !== 32) {
            return {
              valid: false,
              error: `note_enc_pub_key must decode to exactly 32 bytes (got ${bytes.length})`,
            };
          }
        } catch (e) {
          return {
            valid: false,
            error: `note_enc_pub_key is not valid base64: ${e.message}`,
          };
        }
      ...
      const ownerKeyValidation = validateOwnerCipherPayPubKey(
        normalizedRecipientKey,
      );
      if (!ownerKeyValidation.valid) {
        console.error(
          "[CipherPayService] Invalid owner_cipherpay_pub_key format:",
          ownerKeyValidation.error,
        );
      ...
      try {
        const response = await fetch(
          `${serverUrl}/api/v1/users/note-enc-pub-key/${normalizedRecipientKey}`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            throw new Error(
              "Invalid recipient! Recipient not found or missing required keys.",
            );
          } else {
            const errorText = await response.text();
            console.error(
              "[CipherPayService] Failed to validate recipient:",
              response.status,
              errorText,
            );
        ...
        if (!data.noteEncPubKey) {
          throw new Error(
            "Invalid recipient! Recipient missing note encryption public key.",
          );
        }
        ...
        if (!noteEncKeyValidation.valid) {
          console.error(
            "[CipherPayService] Invalid note_enc_pub_key format:",
            noteEncKeyValidation.error,
          );
          throw new Error(
            `Invalid recipient! Corrupted note_enc_pub_key: ${noteEncKeyValidation.error}`,
          );
        }

        console.log(
          "[CipherPayService] Recipient validated successfully with valid keys",
        );
      } catch (error) {
        ...
        throw new Error(
          "Invalid recipient! Failed to verify recipient in database.",
        );
      }
      ...
      if (inputNote) {
        ...
        if (inputNoteAmount < MIN_TRANSFER_AMOUNT) {
          throw new Error(
            `Input note amount must be at least 0.001 SOL. Current: ${(Number(inputNoteAmount) / 1e9).toFixed(9)} SOL`,
          );
        }
        ...
        if (spendable.length === 0) {
          throw new Error(
            "No spendable notes available. Please deposit funds first.",
          );
        }

        // Filter out notes that are less than minimum transfer amount (0.001 SOL)
        const validNotes = spendable.filter(
          (n) => BigInt(n.amount) >= MIN_TRANSFER_AMOUNT,
        );
        if (validNotes.length === 0) {
          throw new Error(
            "No notes with sufficient amount (minimum 0.001 SOL) available for transfer.",
          );
        }

        // VALIDATION: Check total available balance first
        const totalAvailable = validNotes.reduce(
        ...
        if (singleNote) {
          console.log(
            "[CipherPayService] Found single note sufficient for transfer:",
            {
          ...
          console.log(
            "[CipherPayService] No single note sufficient, selecting multiple notes...",
          );
  ...
  async executeSingleTransfer(
    ...
    try {
      ...
      if (
        !inputNoteToUse.amount ||
        !inputNoteToUse.tokenId ||
        !inputNoteToUse.ownerCipherPayPubKey ||
        !inputNoteToUse.randomness
      ) {
      ...
      if (inputNoteToUse.amount < amount) {
        throw new Error(
          `Insufficient balance in selected note. Note amount: ${inputNoteToUse.amount}, requested: ${amount}`,
        );
      }
      ...
      if (transferAmount === inputAmount) {
        ...
        const randomPercent =
          minPercent +
          (BigInt(randomBytes[0]) % (maxPercent - minPercent + 1n));
        ...
        out1Amount =
          (out1AmountUnrounded / roundingPrecision) * roundingPrecision;
        ...
        if (
          ...
          const halfAmount =
            (inputAmount / 2n / roundingPrecision) * roundingPrecision;
        ...
        console.log(
          "[CipherPayService] Full amount transfer - Randomly split (rounded to 0.001 SOL), both outputs for recipient:",
          {
        ...
        console.log(
          "[CipherPayService] Partial amount transfer - Direct split:",
          {
            inputAmount: inputAmount.toString(),
            transferAmount: transferAmount.toString(),
            out1Amount: out1Amount.toString(),
      ...
      const { getLocalEncPublicKeyB64, encryptForRecipient, encryptForSender } =
      ...
      const getRecipientNoteEncPubKey = async (ownerCipherPayPubKey) => {
        try {
          const response = await fetch(
            `${serverUrl}/api/v1/users/note-enc-pub-key/${ownerCipherPayPubKey}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
          ...
          if (response.ok) {
            ...
            console.warn(
              `[CipherPayService] Failed to get note_enc_pub_key for ${ownerCipherPayPubKey}:`,
              response.status,
              response.statusText,
            );
            return null;
          ...
          console.error(
            `[CipherPayService] Error fetching note_enc_pub_key for ${ownerCipherPayPubKey}:`,
            error,
          );
          return null;
        }
      ...
      const convertedMemo = inputNoteToUse.memo
        ? toBigInt(inputNoteToUse.memo)
        : 0n;
      ...
      const inputNoteObj = {
        ...
        randomness: {
          ...
          s: inputNoteToUse.randomness.s
            ? toBigInt(inputNoteToUse.randomness.s)
            : undefined,
        },
        memo: convertedMemo,
      };
      ...
      const senderKey =
        "0x" + senderOwnerCipherPayPubKey.toString(16).padStart(64, "0");
      ...
      const out1Recipient = isFullTransfer
        ...
        : recipientGetsOut1
          ...
          : BigInt(inputNoteToUse.ownerCipherPayPubKey);
      ...
      const transferParams = {
        ...
        onOut1NoteReady: async (note) => {
          try {
            const hasNoteTokenId =
              "tokenId" in note &&
              note.tokenId !== undefined &&
              note.tokenId !== null;
            const tokenIdForCommitment = hasNoteTokenId
              ? note.tokenId
              : convertedTokenId;
            ...
            const noteOwnerKey =
              "0x" + note.ownerCipherPayPubKey.toString(16).padStart(64, "0");
            const expectedRecipientKey =
              "0x" + recipientCipherPayPubKey.toString(16).padStart(64, "0");

            if (noteOwnerKey !== expectedRecipientKey) {
              console.error(
                "[CipherPayService] WARNING: Out1 note ownerCipherPayPubKey does not match expected recipient!",
                {
                  noteOwnerKey,
            ...
            const recipientOwnerKey =
              "0x" + note.ownerCipherPayPubKey.toString(16).padStart(64, "0");
            const recipientNoteEncPubKey =
              await getRecipientNoteEncPubKey(recipientOwnerKey);
            ...
            const noteData = {
              note: {
                ...
                tokenId:
                  "0x" +
                  (note.tokenId !== undefined && note.tokenId !== null
                    ? note.tokenId
                    : convertedTokenId
                  ).toString(16),
                ownerCipherPayPubKey:
                  "0x" +
                  note.ownerCipherPayPubKey.toString(16).padStart(64, "0"),
                randomness: {
                  ...
                  ...(note.randomness.s
                    ? {
                        s:
                          "0x" +
                          note.randomness.s.toString(16).padStart(64, "0"),
                      }
                    : {}),
                },
            ...
            const ciphertextB64 = encryptForRecipient(
              recipientEncPubKeyB64,
              noteData,
            );
            ...
            const isOut1ChangeNote =
              !isFullTransfer &&
              note.ownerCipherPayPubKey ===
                BigInt(inputNoteToUse.ownerCipherPayPubKey);
            console.log(
              "[CipherPayService] Out1 note check - isFullTransfer:",
              isFullTransfer,
              "isOut1ChangeNote:",
              isOut1ChangeNote,
              "note.ownerCipherPayPubKey:",
              note.ownerCipherPayPubKey.toString(),
              "inputNote.ownerCipherPayPubKey:",
              inputNoteToUse.ownerCipherPayPubKey,
            );
            ...
            if (!isOut1ChangeNote) {
              ...
              console.log(
                "[CipherPayService] Generating audit receipt for out1 (payment note)",
              );
              try {
                ...
                const hasNoteTokenId =
                  ...
                  note.tokenId !== undefined &&
                  note.tokenId !== null;
                const tokenIdForAudit = hasNoteTokenId
                  ? note.tokenId
                  : convertedTokenId;
                ...
                if (this.sdk && typeof this.sdk.commitmentOf === "function") {
                  try {
                    ...
                    console.warn(
                      "[CipherPayService] Failed to compute commitment for audit receipt:",
                      e,
                    );
                  }
                }
                ...
                if (tokenIdForAudit === undefined || tokenIdForAudit === null) {
                  console.error(
                    "[CipherPayService] ERROR: tokenId is missing!",
                    {
                      note: {
                  ...
                  throw new Error(
                    "tokenId is required for audit receipt but is missing",
                  );
                }

                // Ensure tokenIdForAudit is a BigInt before calling toString
                const finalTokenId =
                  typeof tokenIdForAudit === "bigint"
                    ? tokenIdForAudit
                    : BigInt(tokenIdForAudit);

                // Create audit receipt object
                const randomnessRHex =
                  "0x" + note.randomness.r.toString(16).padStart(64, "0");
                const auditReceipt = {
                  amount: "0x" + note.amount.toString(16),
                ...
                ciphertextAuditB64 = encryptForSender(
                  senderEncPubKeyB64,
                  auditReceipt,
                );
              } catch (error) {
                console.error(
                  "[CipherPayService] ❌ Failed to generate audit receipt for out1 (non-fatal):",
                  error,
                );
              ...
              console.log(
                "[CipherPayService] Skipping audit receipt for out1 note (change note for sender)",
              );
            }
            ...
            if (messageResponse.ok) {
              ...
              if (messageResult.contentHash)
                transferContentHashes.push(messageResult.contentHash);
            } else {
              const errorText = await messageResponse.text();
              console.error(
                "[CipherPayService] ❌ Failed to save out1 note message:",
            ...
            console.error(
              "[CipherPayService] ❌ Exception while saving encrypted out1 note message:",
              error,
            );
        ...
        onOut2NoteReady: async (note) => {
          ...
          if (note.amount === 0n || note.amount === 0) {
            console.log(
              "[CipherPayService] Out2 note has 0 amount, skipping save",
            );
            return;
          }
          ...
          try {
            ...
            if (isChangeNoteForSender) {
              ...
              if (!recipientNoteEncPubKey) {
                throw new Error(
                  `Failed to get note_enc_pub_key for recipient ${recipientOwnerKey}`,
                );
              }
              // Use Curve25519 public key directly (no derivation needed - it's already a Curve25519 key)
            ...
            if (!isChangeNoteForSender) {
              ...
              try {
                ...
                console.error(
                  "[CipherPayService] ❌ Failed to generate audit receipt for out2 (non-fatal):",
                  error,
                );
                console.error("[CipherPayService] Error details:", {
                  message: error.message,
              ...
              console.log(
                "[CipherPayService] Skipping audit receipt for out2 note (change note for sender)",
              );
            }
            ...
            if (messageResponse.ok) {
              ...
              const errorText = await messageResponse.text();
              console.warn(
                "[CipherPayService] Failed to save out2 note message:",
                errorText,
              );
            }
          } catch (error) {
            console.warn(
              "[CipherPayService] Failed to save encrypted out2 note message:",
              error,
            );
          }
        },
      ...
      if (
        result.proofHex &&
        result.proofPublicSignals &&
        transferContentHashes.length > 0
      ) {
        ...
        try {
          ...
          console.warn(
            "[CipherPayService] Failed to store proof on messages:",
            e,
          );
        }
      }
      ...
      console.error(
        "[CipherPayService] Failed to execute single transfer:",
        error,
      );
      throw error;
    }
  ...
  async approveRelayerDelegate(params) {
    try {
      console.log(
        "[CipherPayService] approveRelayerDelegate called with params:",
        params,
      );
      ...
      const decimals =
        params.tokenMint === WSOL_MINT ? 9 : (params.decimals ?? 9);

      // Sanity-check the wallet has enough SOL for fees (proxied read).
      const balanceRes = await fetch(
        `${serverUrl}/api/wallet/balance?pubkey=${walletPublicKey.toBase58()}`,
      );
      const balanceJson = await balanceRes.json();
      if (balanceJson.ok && balanceJson.walletBalance < 5000) {
        throw new Error(
          `Insufficient SOL for transaction fees. Need at least 0.000005 SOL (5000 lamports), have ${balanceJson.walletBalance} lamports.`,
        );
      }
      ...
      const blockhashRes = await fetch(
        `${serverUrl}/api/wallet/latest-blockhash`,
      );
      const blockhashJson = await blockhashRes.json();
      if (!blockhashJson.ok)
        throw new Error(
          blockhashJson.error || "Failed to fetch latest blockhash",
        );
      ...
      if (!submitJson.ok)
        throw new Error(submitJson.error || "Failed to submit transaction");

      console.log(
        "[CipherPayService] Delegate approval completed:",
        submitJson.signature,
      );
      ...
      console.error(
        "[CipherPayService] Failed to approve relayer delegate:",
        error,
      );
      throw error;
    }
  ...
  async checkRelayerDelegateApproved(params) {
    try {
      ...
      const serverUrl = getServerBaseUrl();
      const response = await fetch(
        `${serverUrl}/api/wallet/delegate-approved?wallet=${walletPublicKey}&tokenMint=${tokenMint}`,
      );
      ...
      console.warn(
        "[CipherPayService] checkRelayerDelegateApproved failed:",
        err?.message || err,
      );
      return false;
    }
  ...
  async createDeposit(params) {
    ...
    try {
      console.log(
        "[CipherPayService] createDeposit called with params:",
        params,
      );
      ...
      if (!window.CipherPaySDK?.deposit) {
        throw new Error(
          "SDK deposit function not available. Ensure the SDK bundle is loaded.",
        );
      }
      ...
      const onNoteReady = async (note) => {
        try {
          console.log(
            "[CipherPayService] Note ready, encrypting and saving...",
            note,
          );
            ...
            note: {
              ...
              randomness: {
                ...
                ...(note.randomness.s
                  ? {
                      s:
                        "0x" + note.randomness.s.toString(16).padStart(64, "0"),
                    }
                  : {}),
              },
              ...(note.memo ? { memo: "0x" + note.memo.toString(16) } : {}),
          ...
          console.log(
            "[CipherPayService] Encryption successful, ciphertext length:",
            ciphertextB64.length,
          );
          const recipientKey =
            "0x" + note.ownerCipherPayPubKey.toString(16).padStart(64, "0");
          ...
          if (messageResponse.ok) {
            ...
            console.log(
              "[CipherPayService] Saved encrypted note message during prepare:",
              messageResult,
            );
          } else {
            const errorText = await messageResponse.text();
            console.warn(
              "[CipherPayService] Failed to save note message:",
              errorText,
            );
          ...
          console.warn(
            "[CipherPayService] Failed to save encrypted note message:",
            error,
          );
          // Don't throw - deposit can continue even if message save fails
        }
      ...
      if (depositContentHash && result.proofHex && result.proofPublicSignals) {
        try {
          const patchRes = await fetch(
            `${serverUrl}/api/v1/messages/${depositContentHash}/proof`,
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
          ...
          if (patchRes.ok) {
            ...
            console.warn(
              "[CipherPayService] Failed to store proof on message:",
              await patchRes.text(),
            );
          }
        } catch (e) {
    ...
    try {
      ...
      if (storedIdentity) {
        ...
        console.log("[CipherPayService] Loaded identity with wallet keys:", {
          ownerWalletPubKey:
            ownerWalletPubKey.toString().substring(0, 20) + "...",
          ownerWalletPrivKey:
            ownerWalletPrivKey.toString().substring(0, 20) + "...",
        });
      ...
      console.warn(
        "[CipherPayService] No stored identity found, using temporary identity",
      );
      return {
  ...
  async verifyProof(proof, publicSignals, verifierKey) {
    ...
    try {
      return await this.sdk.zkProver.verifyProof(
        proof,
        publicSignals,
        verifierKey,
      );
  ...
  verifyProofOfPayment(proof, note, viewKey) {
    return (
      this.sdk?.viewKeyManager?.verifyProofOfPayment(proof, note, viewKey) ||
      false
    );
  }
  ...
  async fetchMerkleRoot() {
    ...
    if (!this.sdk?.merkleTreeClient) {
      throw new Error(
        "Merkle Tree Client is not available. It may not be initialized or may require additional configuration.",
      );
    }
  ...
  async getWithdrawableNotes() {
    ...
    try {
      ...
      const validNotes = spendable.filter(
        (n) => BigInt(n.amount) >= MIN_WITHDRAW_AMOUNT,
      );
      ...
      console.error(
        "[CipherPayService] Failed to get withdrawable notes:",
        error,
      );
      throw error;
    }
  ...
  async withdraw(selectedNote, recipientSolanaAddress) {
    ...
    try {
      ...
      if (!recipientSolanaAddress)
        throw new Error("Recipient Solana address is required");

      // Validate note structure
      if (
        !selectedNote.amount ||
        !selectedNote.tokenId ||
        !selectedNote.ownerCipherPayPubKey ||
        !selectedNote.randomness
      ) {
      ...
      if (noteAmount < MIN_WITHDRAW_AMOUNT) {
        throw new Error(
          `Note amount must be at least 0.001 SOL. Current: ${(Number(noteAmount) / 1e9).toFixed(9)} SOL`,
        );
      }
      ...
      console.log(
        "[CipherPayService] Withdrawing full amount of selected note:",
        {
          noteAmount: noteAmount.toString(),
      ...
      const recipientCipherPayPubKey = await poseidonHash([
      ...
      const randomness = BigInt(
        typeof randomnessValue === "object" &&
          randomnessValue !== null &&
          randomnessValue.r !== undefined
          ? randomnessValue.r
          : randomnessValue,
      );
      ...
      console.log(
        "[CipherPayService] Computed commitment for withdraw:",
      ...
      const prepareResponse = await fetch(
        `${serverUrl}/api/v1/prepare/withdraw`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...(prepareAuthToken
              ? { Authorization: `Bearer ${prepareAuthToken}` }
              : {}),
          },
          body: JSON.stringify({
      ...
      try {
        ...
        if (recipientEncPubKeyB64) {
          ...
          const withdrawData = {
            ...
            note: {
              ...
              randomness: {
                ...
                ...(selectedNote.randomness?.s
                  ? {
                      s:
                        "0x" +
                        BigInt(selectedNote.randomness.s)
                          .toString(16)
                          .padStart(64, "0"),
                    }
                  : {}),
          ...
          if (messageResponse.ok) {
            ...
            console.log(
              "[CipherPayService] Saved withdraw message during prepare:",
              messageResult,
            );
          } else {
            const errorText = await messageResponse.text();
            console.warn(
              "[CipherPayService] Failed to save withdraw message during prepare:",
              errorText,
            );
          }
        }
        ...
        console.warn(
          "[CipherPayService] Failed to create withdraw message during prepare:",
          error,
        );
      ...
      if (this.sdk?.zkProver?.generateWithdrawProof) {
        try {
          console.log(
            "[CipherPayService] Generating withdraw proof using SDK zkProver...",
          );
          const proofResult =
            await this.sdk.zkProver.generateWithdrawProof(witnessInputs);
          proof = proofResult.proof;
          publicSignals = proofResult.publicSignals;
          console.log(
            "[CipherPayService] Withdraw proof generated successfully",
          );
        } catch (proofError) {
          console.error(
            "[CipherPayService] Failed to generate proof using SDK zkProver:",
            proofError,
          );
          throw new Error(
            `Failed to generate withdraw proof: ${proofError.message}`,
          );
        }
      } else {
      ...
      const submitResponse = await fetch(
        `${serverUrl}/api/v1/submit/withdraw`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      console.log(
        "[CipherPayService] Withdraw submitted successfully:",
        submitResult,
      );
      ...
      if (withdrawContentHash && proof && publicSignals) {
        try {
          ...
          if (proofHex) {
            const authToken = localStorage.getItem("cipherpay_token");
            const patchRes = await fetch(
              `${serverUrl}/api/v1/messages/${withdrawContentHash}/proof`,
              {
                method: "PATCH",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (patchRes.ok) {
              console.log(
                "[CipherPayService] Stored proof on withdraw message",
              );
            } else {
              console.warn(
                "[CipherPayService] Failed to store proof on withdraw message:",
                await patchRes.text(),
              );
            }
          }
      ...
      return {
        txHash:
          ...
          "pending",
        signature:
          submitResult.signature || submitResult.txid || submitResult.txSig,
  ...
  async getServiceStatus() {
    console.log(
      "[CipherPayService] getServiceStatus called (should always log this!)",
    );
    ...
    console.log("[CipherPayService] getServiceStatus returning:", {
      isConnected,
      publicAddress,
      balance,
    });
  ...
  async getAccountOverviewFromBackend(options = {}) {
    try {
      ...
      console.error(
        "[CipherPayService] Failed to get account overview from backend:",
        error,
      );
      throw error;
    }
  ...
  async getMessagesFromBackend(options = {}) {
    try {
      ...
      console.error(
        "[CipherPayService] Failed to fetch messages from backend:",
        error,
      );
      throw error;
    }
  ...
  async computeAccountOverviewFromNotes(notes, checkOnChain = false) {
    try {
      ...
      console.error(
        "[CipherPayService] Failed to compute account overview:",
        error,
      );
      throw error;
    }
  ...
  emit(eventType, data) {
    ...
    this.eventListeners[eventType].forEach((callback) => {
      try {
        ...
        console.error(
          `[CipherPayService] Error in event listener for ${eventType}:`,
          error,
        );
      }
    });
  ...
  async startEventMonitoring(recipientKey) {
    ...
    if (!recipientKey) {
      console.warn(
        "[CipherPayService] Cannot start event monitoring: recipientKey required",
      );
      return;
    }

    console.log(
      "[CipherPayService] Starting SSE event monitoring for:",
      recipientKey,
    );
    ...
    try {
      ...
      eventSource.onerror = (error) => {
        ...
        if (eventSource.readyState === EventSource.CLOSED) {
          console.log(
            "[CipherPayService] SSE connection closed, stopping monitoring",
          );
          this.eventMonitoringActive = false;
        }
      };
      ...
      console.log(
        "[CipherPayService] SSE event monitoring started successfully",
      );
    } catch (error) {
      console.error(
        "[CipherPayService] Failed to start SSE event monitoring:",
        error,
      );
      this.eventMonitoringActive = false;
    }
